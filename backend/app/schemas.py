...
class GradeItem(BaseModel):
    """批量定级中一条缺陷的定级内容；处理期限留空时按严重等级自动计算。"""
...
class GradeSuggestPayload(BaseModel):
    """批量定级建议的查询内容：按缺陷编号逐条给出建议等级与处理期限。"""

    缺陷编号列表: list[str] = Field(default_factory=list)
